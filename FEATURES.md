...
| 64 | P3 | S | 🔲 | **Expired-session cleanup via Railway cron** | The prod `sessions` table grows forever (~4.8k rows at July 2026) — Flask-Session 0.8.0 never deletes expired rows unless told to. Chosen approach: Railway cron service running `flask session_cleanup` (the command Flask-Session auto-registers **when `SESSION_CLEANUP_N_REQUESTS` is unset** — the two modes are mutually exclusive, so don't also set that config or the command disappears). Setup: a second Railway service on the same repo/env vars with a cron schedule (e.g. daily) and start command `flask session_cleanup`; it needs `DATABASE_URL` + `SECRET_KEY` + `FLASK_ENV`. Deliberately preferred over the in-request `SESSION_CLEANUP_N_REQUESTS=N` one-liner as a learning exercise for Railway cron — the one-liner remains the documented fallback (tail-latency + shared-`db.session` commit caveats discussed 2026-07-06). |
