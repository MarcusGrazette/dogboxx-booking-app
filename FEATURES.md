...
| 63 | P2 | M | 🔲 | **Backup confidence — restore drill + R2 photo reconciliation** | Backups are assumed, not proven. (a) Rehearse a Railway Postgres restore once, into a scratch DB/service (never prod), and write down the steps + timing. (b) Dog photos live on a single Railway volume with best-effort R2 backup (`uploads.py::_backup_to_r2` logs a warning and continues on failure; nothing reconciles afterwards) — add a small script that diffs volume contents against the R2 bucket and reports missing objects. Run it as part of the drill, then occasionally. |
