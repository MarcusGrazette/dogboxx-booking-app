...
| 59 | P2 | L | 🔧 | **July 2026 code-review remediation** | 16 findings (resilience, performance, best practices, UX consistency) being worked through in order. Deployed to prod: #1, #2, #3, #5, #6, #7 (PR #143/#144/#145/#146), #15, #16 (PR #147), #4 static-cache split-policy (PR #148), #9 email env-source + #10 logging.exception sweep (PR #149) — all as of 2026-07-05. Remaining, opportunistic: #8 (onboarding session flag), #11 (301→308 + CSRF referrer), #12 (split client/routes.py), #13 (tz-aware DateTime, new cols only), #14 (recurring-booking lock — scale note, no action). Full findings + status tracker + per-item risk notes in `docs/CODE_REVIEW_2026-07.md` — update that file's checklist as items land. |
