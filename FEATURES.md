...
| 63 | P2 | M | 🔧 | **Backup confidence — restore drill + R2 photo reconciliation** | Backups are assumed, not proven — and auditing this surfaced a real incident (see #63a note). (a) **Postgres restore drill — not yet done, but the plan changed.** Investigating this found the nightly `pg_dump` → R2 GitHub Actions backup had been silently producing empty (20-byte) dumps for ~3 months (pg_dump v16 vs. Railway's Postgres v18, no `pipefail`) — fixed directly (`.github/workflows/backup.yml`, PR #154, verified with a real 856 KB dump). Plan for the actual "restore into a scratch service, never prod" drill is now Railway PITR (Point-in-Time Recovery) — the only Railway restore mechanism that creates a separate sibling service rather than swapping the volume on the live one. PITR not yet enabled — user is enabling it dashboard-side at a quiet time (brief Postgres redeploy required), then we do the drill. (b) **R2 photo reconciliation — implemented on `develop`.** `/api/internal/uploads-manifest` (new route, `app/blueprints/api/routes.py`, gated by `internal_only`/`INTERNAL_API_SECRET` — `app/utils/decorators.py`) lets a sibling service read the uploads volume, since Railway volumes can only mount to one service; `flask reconcile-uploads` (`run.py` + `app/utils/uploads_reconcile.py`) fetches that manifest plus the R2 bucket listing and diffs them, emailing `BUG_REPORTS` only if something's actually missing or size-mismatched. Manually verified end-to-end against the real volume/bucket before writing any code (23/23 dog photos + 9/9 profile photos matched exactly — this mechanism, unlike the DB backup, was already healthy). Needs a new Railway cron service (start command `flask reconcile-uploads`, weekly schedule) with `INTERNAL_API_SECRET` + `WEB_INTERNAL_URL` + the R2_* vars — not yet created. |
