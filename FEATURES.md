...
| 59 | P2 | L | 🔧 | **July 2026 code-review remediation** | 16 findings (resilience, performance, best practices, UX consistency) being worked through in order. Deployed to prod: #1, #2, #3, #5, #6, #7 (PR #143/#144/#145/#146), #15, #16 (PR #147), #4 static-cache split-policy (PR #148), #9 email env-source + #10 logging.exception sweep (PR #149), #8 onboarding session flag + #11 301→308/CSRF referrer (PR #150) — all as of 2026-07-05. #12 (split client/routes.py into views/) done, PR open to develop. Remaining, opportunistic: #13 (tz-aware DateTime, new cols only), #14 (recurring-booking lock — scale note, no action). Full findings + status tracker + per-item risk notes in `docs/CODE_REVIEW_2026-07.md` — update that file's checklist as items land. |
| 60 | P4 | S | 📋 | **Dead-local cleanup found during #12 split** | Two pre-existing (not introduced by the split) unused-variable nits caught by pyflakes while splitting `client/routes.py`: `views/bookings.py::recurring_booking` computes `freq_label`/`slot_label`/`service_label` that are never read (notification text now goes through `NotificationBatch`/`summarise()` instead); `views/profile.py::update_pickup` fetches `client = Client.query...` and never uses it (pickup notes live on `Dog`, not `Client`). Both are inert — no behavior bug — but worth a one-line-each cleanup pass. Kept out of #12 to keep that PR a pure move. |
