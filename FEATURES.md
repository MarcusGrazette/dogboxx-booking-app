...
| 59 | P2 | L | 🔧 | **July 2026 code-review remediation** | 16 findings (resilience, performance, best practices, UX consistency) being worked through in order. #1 (psycogreen + pool_pre_ping) and #2 (deactivation session revocation) deployed to prod; next is #3 (webpush timeout). Full findings + status tracker + per-item risk notes in `docs/CODE_REVIEW_2026-07.md` — update that file's checklist as items land. |
