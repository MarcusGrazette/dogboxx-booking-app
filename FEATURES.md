...
| 62 | P2 | M | 🔧 | **Production monitoring — Sentry + uptime check** | From the July 2026 big-picture review: prod 500s are only visible by tailing `railway logs` (tracebacks are logged since review #10, but nobody is *notified*), and nothing watches the app between deploys. (a) **Sentry — implemented on `develop`, pending deploy.** `sentry_sdk.init()` guarded on new `SENTRY_DSN` env var (unset in dev/CI/test, so no local noise); `send_default_pii=False` (real client PII, third-party tool); `traces_sample_rate=1.0`; default `LoggingIntegration` left on, so every `logging.error`/`logging.exception` call becomes a Sentry event too, not just unhandled 500s — deliberate choice for max coverage, revisit if specific handled-error call sites turn out noisy. Needs `SENTRY_DSN` set in Railway before it does anything in prod. (b) External uptime monitor (e.g. UptimeRobot free tier) polling `https://app.dogboxx.org/health` — the endpoint already verifies a real table read + Alembic head, so it's a meaningful probe, not just a ping. Still open. |
