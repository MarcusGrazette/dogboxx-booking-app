...
createdb -O dogboxx dogboxx_test   # one-time: test DB on your existing dev Postgres
                                   # (full from-scratch Postgres setup: see README "Set up PostgreSQL")
...
322 tests across auth, bookings, capacity, multi-owner, notifications, drop-in, invoicing, password reset, super-admin, broadcasts, activity feed, closures, walker schedule, bulk-cancel. All should pass. CI runs on every push/PR — don't merge anything that breaks CI.
...
- **`batch_id` is required for activity-feed creation-row suppression**: any route that calls `record_booking_created` and may immediately follow with a `transition_booking` confirm (e.g. auto-confirm via `_maybe_auto_confirm`) **must** generate a shared `batch_id = uuid.uuid4().hex` and pass it to both calls. The activity feed's `_immediate_confirms` set only suppresses the creation row when both BSC rows share a non-None `batch_id` — if either call omits it, the feed shows a spurious "Requested" entry above the "Confirmed" one. Bulk paths (`book_both`, `recurring_booking`, admin `book_for_dog`/`recurring_for_dog`) already follow this pattern; single-booking routes must too.
- **Walker availability change must reset confirmed bookings**: when a walker loses availability for a (date, slot), existing future confirmed bookings on that combo must be reset to `walker_id=None, status='requested'` so they surface as pending for reassignment. Skipping the reset creates UI-orphan bookings: still `confirmed` in DB but render nowhere on `/admin/board` (no lane for the unscheduled walker, not in pending). **The reset is mandatory and every path does it.** All **six** reset paths (`add_unavailability`, `schedule_changes_batch`, `walker_schedule_json`, `admin_add_unavailability`, `deactivate_walker`, and `remove_walker_role`) now also emit a grouped `booking_reset` notification to each affected client (first five: Session 3, PR #117; `remove_walker_role` was missed there and fixed in the review pass, PR #121). Any new path that removes walker availability **must** do both the reset and the `booking_reset` notification via `NotificationBatch`.
...
- **Admin notification fan-out**: all admin notifications use `User.query.filter_by(is_admin=True).all()` — any walker promoted to admin via the toggle on `/admin/walkers` (sets `is_admin=True`) immediately receives the full admin notification stream. They also get full access to `/admin/*` routes. **Admin-walker double-count hazard**: when a route fans out to both admins and assigned walkers, a walker who is also an admin will appear in both loops and accumulate double the payloads in their `NotificationBatch` group — `summarise()` then reports 2× the actual count in the bell. Guard the walker notification with `walker.user_id not in admin_ids` (build `admin_ids = {a.id for a in admins}` before the loop). The co-owner loop already guards this with `if not co_user.is_admin`. Fixed in `pause_walks` (PR #123).
...
- **Rate limiting must not throttle signed-in users, and error pages must be JSON for AJAX** (PR #125): `RATELIMIT_DEFAULT` (`50/hour` per-IP) applies to *every* route. Authenticated users are exempted from the **default** limits via `Limiter(..., default_limits_exempt_when=lambda: current_user.is_authenticated)` — the admin dashboard alone fires several fetch calls per interaction (`month-data`, `day-detail`, `board-data`, `board-chart-data`, `pending-counts`), so without the exemption an active admin exceeds the default in minutes and `assign_walker` returns 429 (worse on a shared office IP). The exemption only affects the catch-all default; explicit `@limiter.limit` decorators (login `5/min`, password reset, report-bug) still fire, so brute-force protection on public endpoints is intact. **Relatedly**: any error handler that can fire on an AJAX/fetch endpoint **must return JSON for non-navigation requests** — the `429`, CSRF, and role-decorator (`admin_required`/`walker_required`) handlers all detect this via `request.is_json or X-Requested-With == 'XMLHttpRequest'` (the 429 handler also treats `Accept` lacking `text/html` as AJAX, to catch `fetch()` GETs that send `Accept: */*`). An HTML error body reaching a client's `res.json()` throws `unexpected token '<'` and surfaces a cryptic toast. Note CSRF validation runs in a `before_request` **before** any view's `@limiter.limit`, so a CSRF-failing request never reaches the limiter. Limiter storage is `memory://` (per-process, resets on restart) unless `REDIS_URL` is set; the new constructor arg is only picked up on a full process restart, not a hot reload.
