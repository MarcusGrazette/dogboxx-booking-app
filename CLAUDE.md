...
| `BookingStatusChange` | Append-only audit log of booking transitions (`from_status` → `to_status`, `changed_by_id`, `notes`, `batch_id`, `created_at`; relationship `Booking.status_history`). **Written at every transition since Session 1 (PR #114)** via the `app/utils/booking_status.py` chokepoint — see Workflow Notes → "Booking status transitions". `batch_id` (migration `6913631b986e`) correlates rows from one bulk action for future feed clustering; nothing reads it yet (Session 4/5). `/admin/activity` does **not** yet read this log — that rebuild is Session 4 (`docs/NOTIFICATIONS.md` §9.6). |
...
- **Flask-Session writes to a stray SQLite file (KNOWN BUG — next #107 slice, not yet fixed)**: `SESSION_TYPE="sqlalchemy"` (config.py) + `Session(app)` (`app/__init__.py:~84–89`) is supposed to store sessions in the app's Postgres `db`, but it materializes the `sessions` table in a gitignored `instance/app.db` SQLite file instead (confirmed: that file contains only a `sessions` table). `instance/app.db` is exactly where Flask resolves the relative `sqlite:///app.db` URI — the `DevelopmentConfig` fallback when `DATABASE_URL` is unset — so the prime suspect is the Session engine binding to that fallback rather than the Postgres `db` (init-ordering around `db.init_app(app)` → `SESSION_SQLALCHEMY = db` → `Session(app)`). **Prod footgun**: session rows in an ephemeral SQLite file are lost on redeploy and not shared across gunicorn workers. This is the second half of issue #107 (the test-default flip was PR #115); investigate + fix in its own branch/PR with its own testing.
