...
- **Conftest fixtures only flush, never commit**: `client_user`, `dog`, `service_type` etc. in `tests/conftest.py` call `_db.session.flush()` but not `commit()`. The autouse `db` fixture wraps the test in one app context so the flushed rows are visible to that session. **Do not open a nested `with app.app_context():` in a test body** — the nested context gets a fresh scoped session that can't see the parents, and any commit inside it will fail FK checks on Postgres CI (e.g. `bookings_user_id_fkey`). SQLite locally is lax enough to hide the bug. Either stay in the outer context, or follow the alternate pattern in `test_admin_assign_walker.py`/`test_admin_client_edit.py` which builds its own helpers that always commit.
