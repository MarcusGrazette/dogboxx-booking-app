...
| 12 | `client/routes.py` split into `views/` package | 🚧 in progress | Branch `feature/client-views-split` — `client/routes.py` (1998 lines) split into `views/general.py`, `views/bookings.py`, `views/profile.py`, `views/onboarding.py`, following the admin split (PR #139). All 22 endpoint names preserved (`client.index`, `client.book`, etc.) — verified via `app.url_map`. 392 tests pass on Postgres. Also fixed a stale `admin.routes`/`client.routes` doc reference in `pricing.py`'s module docstring while touching the area. Not yet PR'd/merged. |
