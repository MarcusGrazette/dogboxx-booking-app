...
| 12 | `client/routes.py` split into `views/` package | ✅ deployed | PR #151 (→develop) + PR #152 (→main), merged + deployed 2026-07-05 — `client/routes.py` (1998 lines) split into `views/general.py`, `views/bookings.py`, `views/profile.py`, `views/onboarding.py`, following the admin split (PR #139). All 22 endpoint names preserved (`client.index`, `client.book`, etc.) — verified via `app.url_map` and a repo-wide grep of every `url_for('client.*')` call site. 392 tests pass on Postgres. Also fixed a stale `admin.routes`/`client.routes` doc reference in `pricing.py`'s module docstring, and (follow-up commit `39aa80e`) removed two pre-existing dead locals pyflakes caught during the split (FEATURES #60). Logs verified clean post-deploy: gevent boot, `/health` 200, SSE Redis listener subscribed on both workers, live traffic through the new modules (e.g. `POST /switch-view` 302) with no errors. |
